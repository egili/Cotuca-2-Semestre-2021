		...
		while(true) 
		{
			...
			synchronized (jogadores)
			{
				if (jogadores.size() == 3 && !isComecou)
				{
					...
					try
					{
						ComunicadoDeDados dados = new ComunicadoDeDados();
						for (Parceiro usuario : jogadores)
							usuario.receba(new ComunicadoDeComecoPartida()); // espera o de comeco
						jogadores.get(0).receba( new ComunicadoDeVez(dados));
					} catch (Exception e) {
						System.err.println("erro na linha 98");
					}
...
//					{
//						if (jogadores.size() == 2)
//						jogadores.get(1).receba(new ComunicadoDeComecoPartida());
//						else
//							if (jogadores.size() == 3)
//								jogadores.get(2).receba(new ComunicadoDeComecoPartida());
//					}
//					catch (Exception e)
//					{
//						System.err.println("erro na linha 115");
//					}
//				}
			}
